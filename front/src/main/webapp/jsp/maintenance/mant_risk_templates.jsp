					...
					<tr>
						...
						<td>
							<div style="margin: 0px auto; width: 80px;">
								<div style="display:none;" class="riskValue">${template.riskRating}</div>
								<div style="width: 25%; float: left; margin-left: 10px;"
									<c:choose>
										<c:when test="${template.riskRating gt 1500 }">
											class="riskRating risk_high"
										</c:when>
										<c:when test="${template.riskRating gt 500 }">
											class="riskRating risk_medium"
										</c:when>
										<c:when test="${template.riskRating eq 0 }">
											class="riskRating"
										</c:when>
										<c:otherwise>
								...
								<c:if test="${template.riskRating != 0}">
									<div style="float: left; margin-left: 10px;">(${template.riskRating})</div>
								</c:if>
							</div>
						</td>
