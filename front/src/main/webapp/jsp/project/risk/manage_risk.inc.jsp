...
function classRiskRating (idDivRiskRating, riskRate) {
	...
	if (riskRate > 1500) {
		$("#" + idDivRiskRating + " div").addClass('risk_high');
	}
	else if (riskRate > 500) {
		$("#" + idDivRiskRating + " div").addClass('risk_medium');
	}
	else if (riskRate == 0) {
		$("#" + idDivRiskRating + " div").removeClass();
	}
	else {
		$("#" + idDivRiskRating + " div").addClass('risk_low');
	}
	...
	riskAjax.call(params, function (data) {
		...
		$(data.historicrisks).each(function() {
			...
			var historicRisk = [
			    ...
			    '<div style="display:none;" class="riskValue">'+(parseInt(this.<%= Historicrisk.PROBABILITY %>) * parseInt(this.<%= Historicrisk.IMPACT %>)) + '</div>' +
