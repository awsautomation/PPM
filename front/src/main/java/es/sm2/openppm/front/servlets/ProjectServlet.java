...
import es.sm2.openppm.core.filters.ProjectFilter;
...
//							datas.add(data[i][j]);
	...
	private void filterTableInvestmentsJX(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		try {
			...
			// Load Filters
			loadFilters(projectFilter, req);
			
			// Find projects
			List<ProjectWrap> projects = projectLogic.findProjects(projectFilter);
	...
	private void filterTableProjectsJX(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		try {
			...
			projectFilter.setShowInactivated(SettingUtil.getBoolean(getSettings(req), Settings.SETTING_SHOW_INACTIVATED, Settings.SETTING_SHOW_INACTIVATED_DEFAULT));
	 ...
	 * @throws Exception 
	 */
	private void loadFilters(ProjectFilter projectFilter, HttpServletRequest req) throws Exception {
		...
		if (SecurityUtil.isUserInRole(req, Constants.ROLE_PMO) || SecurityUtil.isUserInRole(req, Constants.ROLE_FM)) {	// PMO or Functional Manager
			...
			projectFilter.setPerformingorgs(DataUtil.toList(getUser(req).getPerformingorg().getIdPerfOrg()));
		}
		else if (SecurityUtil.isUserInRole(req, Constants.ROLE_PORFM)) {	// Porfolio Manager
			
			projectFilter.setCompany(getCompany(req));
		}
		else if (SecurityUtil.isUserInRole(req, Constants.ROLE_IM)) {	// Investment Manager
			
			projectFilter.setEmployeeByInvestmentManager(getUser(req));
		}
		else if (SecurityUtil.isUserInRole(req, Constants.ROLE_PM)) {	// Project Manager
			
			projectFilter.setEmployeeByProjectManagers(DataUtil.toList(getUser(req).getIdEmployee()));
		}
		else if (SecurityUtil.isUserInRole(req, Constants.ROLE_SPONSOR)) {	// Sponsor
			
			projectFilter.setEmployeeBySponsors(DataUtil.toList(getUser(req).getIdEmployee()));
		}
		else if (SecurityUtil.isUserInRole(req, Constants.ROLE_STAKEHOLDER)) {	// Stakeholder
			
			projectFilter.setStakeholder(getUser(req));
		}
		else if (SecurityUtil.isUserInRole(req, Constants.ROLE_PROGM)) {	// Program Manager
			
			projectFilter.setProgramManager(getUser(req));
		}
		
		// Load Filters
		projectFilter.setStatus(ParamUtil.getStringList(req, Project.STATUS, null));
		projectFilter.setInvestmentStatus(ParamUtil.getStringList(req, Project.INVESTMENTSTATUS, null));
		projectFilter.setSince(ParamUtil.getDate(req, Projectactivity.PLANINITDATE, getDateFormat(req),null));
		projectFilter.setUntil(ParamUtil.getDate(req, Projectactivity.PLANENDDATE, getDateFormat(req),null));
		projectFilter.setBudgetYear(ParamUtil.getInteger(req, Project.BUDGETYEAR, null));
		
		String internalProject = ParamUtil.getString(req, Project.INTERNALPROJECT, null);
		projectFilter.setInternalProject(ValidateUtil.isNull(internalProject) ? null : Boolean.parseBoolean(internalProject));
		
		String isGeoSelling = ParamUtil.getString(req, Project.ISGEOSELLING, null);
		projectFilter.setIsGeoSelling(ValidateUtil.isNull(isGeoSelling) ? null : Boolean.parseBoolean(isGeoSelling));
		
		projectFilter.setCustomertypes(ParamUtil.getIntegersSplit(req, "customertype.idCustomerType"));
		projectFilter.setCustomers(ParamUtil.getIntegersSplit(req, "customer.idCustomer"));
		projectFilter.setPrograms(ParamUtil.getIntegersSplit(req, "program.idProgram"));
		projectFilter.setSellers(ParamUtil.getIntegersSplit(req, "seller.idSeller"));
		projectFilter.setCategories(ParamUtil.getIntegersSplit(req, "category.idCategory"));
		projectFilter.setGeography(ParamUtil.getIntegersSplit(req, "geography.idGeography"));
		projectFilter.setProjects(ParamUtil.getIntegersSplit(req, Project.IDPROJECT));
		
		if (SettingUtil.getBoolean(req, Settings.SETTING_DISABLE_PROJECT, Settings.DEFAULT_DISABLE_PROJECT)) {
			projectFilter.setIncludeDisabled(false); //TODO put filter
		}
		
		if (ValidateUtil.isNull(projectFilter.getEmployeeByProjectManagers())) {
			projectFilter.setEmployeeByProjectManagers(ParamUtil.getIntegersSplit(req, "employeeByProjectManager.idEmployee"));
		}
		
		if (ValidateUtil.isNull(projectFilter.getEmployeeBySponsors())) {
			projectFilter.setEmployeeBySponsors(ParamUtil.getIntegersSplit(req, "employeeBySponsor.idEmployee"));
		}
		
		if (ValidateUtil.isNull(projectFilter.getPerformingorgs())) {
			projectFilter.setPerformingorgs(ParamUtil.getIntegersSplit(req, "performingorg.idPerfOrg"));
		}
		
		projectFilter.setIncludeRejected(JX_FILTER_TABLE.equals(ParamUtil.getString(req, "accion")) ? 
		...
		projectFilter.setProjectName(ParamUtil.getString(req, Project.PROJECTNAME));
		projectFilter.setRag(ParamUtil.getString(req, Project.RAG));
		projectFilter.setPriority(ParamUtil.getString(req, "filterPriority", null));
		projectFilter.setFirstPriority(ParamUtil.getInteger(req, "first", null));
		projectFilter.setLastPriority(ParamUtil.getInteger(req, "last", null));
		projectFilter.setLabels(ParamUtil.getIntegersSplit(req, "label.idLabel"));
		projectFilter.setStageGates(ParamUtil.getIntegersSplit(req, "stageGate.idStageGate"));
		
		// Filters by configuration
		//
		projectFilter.setContractTypes(ParamUtil.getIntegersSplit(req, Configurations.LIST_FILTERS_CONTRACT_TYPE));
		projectFilter.setEmployeeByFunctionalManagers(ParamUtil.getIntegersSplit(req, Configurations.LIST_FILTERS_FM));
		...
		if (ValidateUtil.isNotNull(isIndirectSeller)) {
			projectFilter.setIsIndirectSeller(Boolean.parseBoolean(isIndirectSeller));
		}
		
		// Classification level
		projectFilter.setClassificationsLevel(ParamUtil.getIntegersSplit(req, Configurations.LIST_FILTERS_CLASSIFICATION_LEVEL));
        ...
        projectFilter.setClassificationsLevelUnclassified(classificationsLevel != null && classificationsLevel.contains(Constants.UNCLASSIFIED));

        // Founding source
        projectFilter.setFundingsources(ParamUtil.getIntegersSplit(req, Configurations.LIST_FILTERS_FOUNDING_SOURCE));
        ...
        projectFilter.setFundingSourceUnclassified(fundingSource != null && fundingSource.contains(Constants.UNCLASSIFIED));
		...
		if (ValidateUtil.isNotNull(includingAdjustament)) {
			projectFilter.setIncludingAdjustament(Boolean.parseBoolean(includingAdjustament));
		}
		...
		if (ValidateUtil.isNotNull(riskRating)) {
			...
			projectFilter.setRiskRating(riskRating);
			projectFilter.setFirstRiskRating(ParamUtil.getInteger(req, Configurations.FILTER_FIRST_RISK_RATING, null));
			projectFilter.setLastRiskRating(ParamUtil.getInteger(req, Configurations.FILTER_LAST_RISK_RATING, null));
		}
...
//    	String code = ParamUtil.getString(req, "code", null);
    ...
    private void saveDocumentJX(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		try {
			...
			// Logic
			// FIXME type of document generic
			Documentproject doc = documentprojectLogic.saveDocumentproject(
					idDocumentProject,
					idProject,
					(idClosureCheckProject == -1?type:Constants.DOCUMENT_CLOSURE_CHECK_LIST),
					link,
					comment,
					creationContact,
					creationDate
			...
			if (idClosureCheckProject != -1) {
				...
				// Logic get closure check project
				Closurecheckproject closurecheckproject = closurecheckprojectLogic.findById(idClosureCheckProject);

				// Set document
				closurecheckproject.setDocumentproject(doc);

				// Logic update closure check project
				closurecheckprojectLogic.save(closurecheckproject);

				docJSON.put(Closurecheckproject.IDCLOSURECHECKPROJECT, closurecheckproject.getIdClosureCheckProject());
			}
	...
	private void viewCreateProjectForm(HttpServletRequest req,
		...
		if (SecurityUtil.isUserInRole(req, Constants.ROLE_PMO) ||
			...
			try {
				...
				req.setAttribute("categories", categoryLogic.findByRelation(Category.COMPANY, getCompany(req), Category.NAME, Order.ASC));
	...
	throws ServletException, IOException {
		...
		//TODO esta parte se ha de hacer generica
		int idClosureCheckProject 	= -1;
		if (getMultipartField(Closurecheckproject.IDCLOSURECHECKPROJECT) != null &&
				ValidateUtil.isNotNull(getMultipartField(Closurecheckproject.IDCLOSURECHECKPROJECT))) {
			
			idClosureCheckProject = Integer.parseInt(getMultipartField(Closurecheckproject.IDCLOSURECHECKPROJECT));
		}
		...
		try {			
			...
			doc.setType(idClosureCheckProject == -1?docType:Constants.DOCUMENT_CLOSURE_CHECK_LIST); // FIXME se tiene que modificar
			...
			if (dataFile != null && dataFile.getSize() > 0) {
                ...
                errorFile = validateFile(new File(docFolder), getResourceBundle(req));
			...
			// Set document into closure check project //TODO pendiente del modulo generico
			if (idClosureCheckProject != -1) {
	...
	throws ServletException, IOException {
		...
		try {
            ...
            if (file.exists()) {
                ...
                // Constructor message
                ParamResourceBundle messageError = new ParamResourceBundle("msg.error.file_not_found", true, docFolder);
	...
	throws ServletException, IOException {
		...
		try {
            ...
            // Validate file and permissions
            if (ValidateUtil.isNull(validateFile(file, getResourceBundle(req)))) {
     ...
     * @return
     */
    private String validateFile(File file, ResourceBundle resourceBundle) {
        ...
        if (!file.exists()) {
            ...
            error = paramResourceBundle.getMessage(resourceBundle);
        }
