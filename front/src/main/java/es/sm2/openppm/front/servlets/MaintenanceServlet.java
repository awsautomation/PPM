 ...
 */
	...
	private void updateEmployeeStateJX(HttpServletRequest req,
    	...
    	try {
    		...
    		out.print(info(getResourceBundle(req), StringPool.SUCCESS, "msg.info.change_he", new JSONObject(), "employee", JX_DISABLE_EMPLOYEE.equals(accion)?"disabled":"enabled"));
    	}
		catch (Exception e) { ExceptionUtil.evalueExceptionJX(out, req, getResourceBundle(req), LOGGER, e); }
		finally { out.close(); }
	}
	...
	private void saveEmployee(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if(isUpdate || !contactLogic.hasProfile(getUser(req).getPerformingorg(), new Contact(idContact), 
				...
				if (idResourcePool != null) { employee.setResourcepool(new Resourcepool(idResourcePool)); }			
				...
				if (employee.getDisable() == null) { employee.setDisable(false); }
