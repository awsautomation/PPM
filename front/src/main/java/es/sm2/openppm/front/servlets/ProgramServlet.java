	...
	private void consGantt(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		try {
			...
			if (projects == null || projects.isEmpty()) {
				...
				returnJSON = ChartLogic.consChartGantt(getResourceBundle(req), projects, since, until);
		...
		try {
			...
			if (ids != null && ids.length > 0) {
				...
				if (investiment) {
					...
					chartJSON = ChartLogic.createBubbleChart(getResourceBundle(req), projList, Project.BAC, axesXType, adjustmentApply, idRiskCategories);
				}
        ...
        try {
			...
			if (ids != null) {
				...
				for (Project project : projects) {
					...
					file.addValue(ValidateUtil.toPercent(project.getPoc()));
