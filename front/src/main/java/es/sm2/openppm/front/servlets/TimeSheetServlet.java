    		...
    		if (ids != null) {
    			...
    			for (int idTimeSheet : ids) {
    	    		...
    	    		if (Constants.TIMESTATUS_APP0.equals(timesheet.getStatus())) {
	    	    		...
	    	    		// Logic save
                        timesheetLogic.updateTimeSheet(getUser(req), getResourceBundle(req), getSettings(req), getExtraData(req), approve, timesheet, comments, total);
                    }
    			}
    		}
		}
	...
	private void updateTimeSheetJX(HttpServletRequest req,
    	...
    	try {
            ...
            JSONObject returnJSON = timesheetLogic.updateTimeSheet(getUser(req), getResourceBundle(req),
                    getSettings(req), getExtraData(req), approve, timesheet, comments, null);
