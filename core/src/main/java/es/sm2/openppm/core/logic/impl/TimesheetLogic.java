...
import es.sm2.openppm.core.dao.EmployeeoperationdateDAO;
...
import es.sm2.openppm.core.dao.TimesheetcommentDAO;
	...
	public void changeHours(Employee user, Integer[] idEmployees, Integer[] idProjects,
		...
		try {
			...
			if (ValidateUtil.isNotNull(idEmployees)) {
				...
				String previousStatus = Constants.TIMESTATUS_APP2;
				...
				String nextStatus;
				String prevStatusRej;
				
				Employee filterUser = (approveRol == user.getResourceprofiles().getIdProfile())?user:null;
				
				if (approve) {
					
					nextStatus = (Constants.ROLE_PM  == user.getResourceprofiles().getIdProfile() && approveRol > 0
							? Constants.TIMESTATUS_APP2 : Constants.TIMESTATUS_APP3);
					
					if (approveRol > 0) {
						prevStatusRej = previousStatus;
					}
					else {
						prevStatusRej = Constants.TIMESTATUS_APP2;
					}
				}
				else { // Rejected
					...
					if ((approveRol > 0 && Constants.ROLE_PM  != user.getResourceprofiles().getIdProfile()) ||
							(approveRol == 0 && Constants.ROLE_PM == user.getResourceprofiles().getIdProfile())) {
	...
	public void changeHours(Employee user, Integer[] idTimeSheets, String comments, boolean approve, 
		...
		try {
			...
			if (ValidateUtil.isNotNull(idTimeSheets)) {
                ...
                // Last level for approve by setting
...
//                    // Role last level be approved hours app1
//                    //
//                    if ((settings != null && SettingUtil.getBoolean(settings, GeneralSetting.LAST_LEVEL_APPROVED_APP1)) &&
//                            lastLevelForApprove.equals(String.valueOf(user.getResourceprofiles().getIdProfile())) &&
//                            timesheet.getOperation() == null) {
//
//                        previousStatus = Constants.TIMESTATUS_APP1;
//                    }
//                    // Approve operation
//                    else if ((settings != null && SettingUtil.getBoolean(settings, GeneralSetting.LAST_LEVEL_APPROVED_APP1)) &&
//                            lastLevelForApprove.equals(String.valueOf(user.getResourceprofiles().getIdProfile())) &&
//                            timesheet.getOperation() != null) {
//
...
//                    }
    	    		...
    	    		if (timesheet != null
    	    				...
    	    				&& (previousStatus.equals(timesheet.getStatus()) || prevStatusRej.equals(timesheet.getStatus()))) {
    ...
    public JSONObject updateTimeSheet(Employee user, ResourceBundle resourceBundle, HashMap<String, String> settings,
									  Map<String, Object> extraData, boolean approve, Timesheet timesheet, String comments, Double total) throws Exception {
		...
		try {
			...
			if ((approve && total == null) || (approve && total > 0)) {
			...
			// Operation
			//
			if (timesheet.getOperation() != null &&
					timesheet.getOperation().getAvailableForManager() != null &&
					timesheet.getOperation().getAvailableForManager()) {
